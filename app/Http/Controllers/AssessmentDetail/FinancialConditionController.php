...
class FinancialConditionController extends Controller
{
    ...
    public function update(Request $request, Assessment $assessment)
    {
        ...
        Financial_indicator::updateOrCreate(
            ['assessment_id' => $assessment->id],
            $request->only([
                'total_revenue', 'total_pad', 'pad_growth', 'pad_revenue',
                'transfer_revenue', 'other_total_revenue', 'volatil_pad',
                'operation_revenue', 'surplus_deficit_before_financing',
                'capital_spending', 'employee_spending', 'pad_last_three_year',
                'total_debt', 'debt_gdp', 'debt_revenue', 'ds_revenue', 'dscr',
                'fiscal_capacity'
            ])
        );

        return redirect()->back()->with('message', 'Financial Condition updated successfully!');
    }
}
